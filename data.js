import { completeGuideCoverage } from './guide-content.js?v=precalculus-8';
        ...
        },
        {
            ...
            lessons: [
                ...
                lesson("2. Tangent, cotangent, secant, and cosecant", "Use zeros and asymptotes as the skeleton.", `
                    <p>Tangent and cotangent have period $\\pi/|b|$. A transformed tangent crosses its midline at $(h,k)$; cotangent has an asymptote at its phase shift. Secant is the reciprocal of cosine and cosecant is the reciprocal of sine, so their asymptotes occur where the partner function is zero.</p>
        ...
        },
        {
            ...
            lessons: [
                lesson("1. Choosing a triangle law", "Match the method to the known parts.", `
                    ...
                    ${note("Worked example", "<p>With sides $13,24,16$, use Law of Cosines to find the largest angle first. Solving gives approximately $A=30.31^\\circ$, $B=111.28^\\circ$, and $C=38.40^\\circ$.</p>")}
...
precalculusData.units[0].questions.push(
    ...
    rq(1,4,"Exponential functions","For $f(x)=-2^x+3$, which domain, range, and end behavior are correct?",["Domain $(-\\infty,\\infty)$; range $(-\\infty,3)$; $f(x)\\to3$ as $x\\to-\\infty$ and $f(x)\\to-\\infty$ as $x\\to\\infty$","Domain $(0,\\infty)$; range $(3,\\infty)$","Domain all real; range $(3,\\infty)$; $f(x)\\to-\\infty$ as $x\\to\\infty$","Domain $(-\\infty,3)$; range all real"],0,"$2^x$ is always positive, so $-2^x+3<3$. As $x\\to-\\infty$, $2^x\\to0$ and the graph approaches $y=3$; as $x\\to\\infty$, the function falls without bound."),
    rq(1,5,"Logarithms","For $f(x)=\\ln(x+2)+1$, which statement is correct?",["Domain $(-2,\\infty)$, range all real, x-intercept $e^{-1}-2$","Domain $[-2,\\infty)$, range $[1,\\infty)$, x-intercept $-1$","Domain all real, range $(-2,\\infty)$","Domain $(2,\\infty)$, x-intercept $e-2$"],0,"The argument requires $x+2>0$. Set $0=\\ln(x+2)+1$ to get $x+2=e^{-1}$. Also $f(x)\\to\\infty$ as $x\\to\\infty$."),
    rq(1,6,"Logarithms","Evaluate, where $\\log$ means base 10: $\\log100,\\ \\ln(e^5),\\ \\log_3(1/9),\\ 6^{\\log_6 36},\\ e^{2\\ln3}$.",["$2,5,-2,36,9$","$100,5,2,6,6$","$2,e^5,-3,36,6$","$10,5,-2,216,9$"],0,"Apply inverse properties and exponent rules: $e^{2\\ln3}=e^{\\ln9}=9$.","Core"),
    rq(1,7,"Logarithms","Assume $x>0$ and $y>0$. Expand $\\log\\left(\\frac{x^2}{5\\sqrt y}\\right)$ completely.",["$2\\log x-\\log5-\\frac12\\log y$","$\\log x^2-5\\log y$","$2\\log x+\\log5+\\frac12\\log y$","$\\frac{2\\log x}{5\\log y}$"],0,"A quotient becomes subtraction and powers become coefficients."),
    rq(1,8,"Logarithms","Assume $z>0$, $x>0$, and $y\\ne0$. Condense $\\frac13\\ln z-2\\ln(5x)-3\\ln(y^2)$.",["$\\ln\\left(\\frac{\\sqrt[3]z}{25x^2y^6}\\right)$","$\\ln\\left(\\frac{z^3}{10xy^2}\\right)$","$\\ln(\\sqrt[3]z-25x^2-y^6)$","$\\ln\\left(\\frac{25x^2y^6}{\\sqrt[3]z}\\right)$"],0,"Move coefficients to powers, then combine subtraction as division."),
...
precalculusData.units[1].questions.push(
    ...
    rq(2,19,"Trig equations","Find all values of $\\theta$ that satisfy each equation over its stated interval:<br><strong>a)</strong> $\\tan\\theta=-1$, $0^\\circ<\\theta\\le360^\\circ$<br><strong>b)</strong> $\\sin\\theta=\\frac{\\sqrt3}{2}$, $0<\\theta\\le2\\pi$<br><strong>c)</strong> $\\cos\\theta=-\\frac12$, $0^\\circ\\le\\theta<360^\\circ$<br><strong>d)</strong> $\\sec\\theta=1$, $0\\le\\theta<2\\pi$<br><strong>e)</strong> $\\sin\\theta=0$, $0\\le\\theta\\le2\\pi$<br><strong>f)</strong> $\\cot\\theta$ is undefined, $0\\le\\theta<2\\pi$",["a) $135^\\circ,315^\\circ$; b) $\\pi/3,2\\pi/3$; c) $120^\\circ,240^\\circ$; d) $0$; e) $0,\\pi,2\\pi$; f) $0,\\pi$","a) $45^\\circ,225^\\circ$; b) $\\pi/6,5\\pi/6$; c) $60^\\circ,300^\\circ$; d) $2\\pi$; e) $\\pi$; f) $\\pi/2,3\\pi/2$","a) $135^\\circ$ only; b) $\\pi/3$ only; c) $240^\\circ$ only; d) $0,2\\pi$; e) $0,\\pi$; f) $0$","All six have no solution"],0,"Use reference angles, quadrant signs, and the exact endpoint rules stated for each equation.","Challenge"),
    rq(2,20,"Trig identities","Which are the three Pythagorean identities?",["$\\sin^2x+\\cos^2x=1$, $1+\\tan^2x=\\sec^2x$, $1+\\cot^2x=\\csc^2x$","$\\sin x+\\cos x=1$, $\\tan x+1=\\sec x$, $\\cot x+1=\\csc x$","$\\sin^2x-\\cos^2x=1$, $\\sec^2x+\\tan^2x=1$, $\\csc^2x+\\cot^2x=1$","$\\sin2x+\\cos2x=1$, $\\tan2x+1=\\sec2x$, $\\cot2x+1=\\csc2x$"],0,"These are the three standard Pythagorean identities."),
...
precalculusData.units[3].questions.push(
    ...
    rq(4,66,"Conic sections","Sketch $(x+2)^2+y^2=9$, then match it to its graph description.",["Circle centered $(-2,0)$ with radius 3","Circle centered $(2,0)$ with radius 9","Parabola with vertex $(-2,0)$","Ellipse centered at the origin"],0,"Circle standard form is $(x-h)^2+(y-k)^2=r^2$."),
    rq(4,67,"Conic sections","Sketch $y=(x-3)^2-5$, then match it to its graph description.",["Upward parabola with vertex $(3,-5)$","Downward parabola with vertex $(-3,5)$","Exponential with asymptote $y=-5$","Circle of radius 5"],0,"Vertex form reads directly as $(h,k)=(3,-5)$."),
    ...
    rq(4,94,"Trig graphs","For $f(x)=-\\cos(x-\\pi)$, which equivalent parent behavior does the function have?",["$\\cos x$","$-\\cos x$","$\\sin x$","$-\\sin x$"],0,"$\\cos(x-\\pi)=-\\cos x$, and the outside negative makes $\\cos x$."),
...
precalculusData.units[4].questions.push(
    rq(5,34,"Triangles","Calculator permitted. Solve each triangle completely, if a triangle exists. If two triangles exist, solve both:<br><strong>a)</strong> $a=13, b=24, c=16$<br><strong>b)</strong> $a=41, c=32, C=20^\\circ$<br><strong>c)</strong> $b=49, c=83, B=38^\\circ$<br><strong>d)</strong> $a=35, c=31, A=52^\\circ$<br><strong>e)</strong> $a=38.5, c=31, B=24^\\circ$<br>Which answer choice correctly summarizes the results?",["a) $A\\approx30.31^\\circ,B\\approx111.28^\\circ,C\\approx38.40^\\circ$; b) $(A_1,B_1,b_1)\\approx(25.99^\\circ,134.01^\\circ,67.29)$ or $(154.01^\\circ,5.99^\\circ,9.76)$; c) no triangle; d) $(B,C,b)\\approx(83.74^\\circ,44.26^\\circ,44.15)$; e) $(A,C,b)\\approx(104.92^\\circ,51.08^\\circ,16.21)$","a) no triangle; b) one; c) two; d) none; e) two","All five have exactly one triangle","All five require only right-triangle trig"],0,"SSS in (a) uses the Law of Cosines. SSA in (b) gives two valid angle supplements, while (c) gives an impossible sine value. In (d) only one supplement fits the angle sum; SAS in (e) uses the Law of Cosines first.","Challenge")
);

// Unit 6: review 35-44, 54-56, and 103-105.
precalculusData.units[5].questions.push(
    rq(6,35,"Complex numbers","Plot $z=-5-12i$ on the complex plane, then find $|z|$.",["$13$","$17$","$7$","$\\sqrt{119}$"],0,"The modulus is $\\sqrt{(-5)^2+(-12)^2}=\\sqrt{169}=13$."),
...
precalculusData.units[6].questions.push(
    ...
    rq(7,49,"Sequences","A geometric sequence has a positive common ratio, $g_2=80000$, and $g_6=32768$. Find $g_7$.",["$26214.4$","$13421.8$","$20971.52$","$16777.216$"],0,"$r^4=32768/80000=0.4096$, so $r=0.8$ and $g_7=32768(0.8)$."),
...
precalculusData.units[7].questions.push(
    ...
    rq(8,58,"Parametric equations","For $x=\\sqrt{t-4}$ and $y=-2t+3$, what restrictions are placed on $t$, $x$, and $y$?",["$t\\ge4,\\ x\\ge0,\\ y\\le-5$","$t\\le4,\\ x\\ge0,\\ y\\ge-5$","$t\\ge4,\\ x\\le0,\\ y\\le-5$","No restrictions"],0,"The square root requires $t\\ge4$ and produces $x\\ge0$; then $y=-2t+3\\le-5$."),
    ...
    rq(8,63,"Polar coordinates","Sketch $r=-6\\cos\\theta$, then choose its complete description.",["Circle centered $(-3,0)$ with radius 3","Circle centered $(3,0)$ with radius 6","Four-petal rose","Limaçon with inner loop"],0,"Multiply by $r$: $r^2=-6r\\cos\\theta$, so $x^2+y^2=-6x$ and complete the square."),
    rq(8,64,"Polar coordinates","Sketch $r=3+4\\sin\\theta$, then choose its complete description.",["A y-axis-symmetric limaçon with an inner loop","A circle centered at $(0,3)$","A four-petal rose","A cardioid with no inner loop"],0,"Because $|4|>|3|$, the limaçon has an inner loop; sine gives y-axis symmetry."),
    rq(8,65,"Polar coordinates","Sketch $r=5\\cos2\\theta$, then choose its complete description.",["A four-petal rose with petals on the coordinate axes","A two-petal rose","A circle of radius 5","A four-petal rose rotated $45^\\circ$"],0,"Even $n=2$ gives $2n=4$ petals; cosine places them on the axes.")
);
...
completeGuideCoverage(precalculusData);
